...
std::tuple<torch::Tensor, torch::Tensor, torch::Tensor> env_shade_fwd(
    ...
    float shadow_scale)
{
    ...
    torch::Tensor resi = torch::zeros({ ro.size(0), ro.size(1), ro.size(2), 2 }, opts) ;
    ...
    params.resi         = packed_accessor32<float, 4>(resi);
    ...
    return std::tuple<torch::Tensor, torch::Tensor, torch::Tensor>(diff, spec, resi);
}
