...
__device__ float shadow_test(uint3 idx, float3 ray_origin, float3 ray_dir, float resi_grad)
{
...
// Samples the GGX (Trowbridge-Reitz) using the distribution of resiible normals (VNDF).
...
__device__ void process_sample(uint3 idx, float3 ray_origin, float3 ray_dir, float3 gb_pos, float3 gb_normal, float3 gb_view_pos, 
    float3 gb_kd, float3 gb_ks, float pdfSum, float weight, float3 &diff, float3 &spec, float2 &resi, float3 diff_grad, float3 spec_grad)
{
    ...
    resi.x = sum(_diff) / 3 * (1 - V) * mis_weight * weight;
    resi.y = sum(_spec) / 3 * (1 - V) * mis_weight * weight;
}
...
extern "C" __global__ void __raygen__rg()
{
    ...
    float2 resiAccum = make_float2(0.0f, 0.0f);
    ...
    for (int i = 0; i < params.n_samples_x * params.n_samples_x; ++i)
    {
        ...
        process_sample(idx, ray_origin, ray_dir, gb_pos, gb_normal, gb_view_pos, gb_kd, gb_ks, pdf_light + pdf_bsdf, sample_frac, diff, spec, resi, diff_grad, spec_grad);
        diffAccum = diffAccum + diff;
        specAccum = specAccum + spec;
        resiAccum = resiAccum + resi;
    ...
    if (!params.backward)
    {
        ...
        params.resi[idx.z][idx.y][idx.x][0] = resiAccum.x;
        params.resi[idx.z][idx.y][idx.x][1] = resiAccum.y;
    }
}
