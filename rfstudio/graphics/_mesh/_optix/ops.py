...
def _get_plugin():
    ...
    torch.utils.cpp_extension.load(name='rf_optixutils_plugin', sources=source_paths, extra_cflags=opts,
         extra_cuda_cflags=opts, extra_ldflags=ldflags, extra_include_paths=include_paths, with_cuda=True, verbose=False)

    # Import, cache, and return the compiled module.
    import rf_optixutils_plugin
    _cached_plugin = rf_optixutils_plugin
    return _cached_plugin
...
class _optix_env_shade_func(torch.autograd.Function):
    ...
    def forward(ctx, optix_ctx, mask, ro, gb_pos, gb_normal, gb_view_pos, gb_kd, gb_ks, light, pdf, rows, cols, BSDF, n_samples_x, rnd_seed, shadow_scale):
        ...
        diff, spec, resi = _get_plugin().env_shade_fwd(optix_ctx.cpp_wrapper, mask, ro, gb_pos, gb_normal, gb_view_pos, gb_kd, gb_ks, light, pdf, rows, cols, _optix_env_shade_func._random_perm[n_samples_x], BSDF, n_samples_x, _rnd_seed, shadow_scale)
        ...
        return diff, spec, resi

    @staticmethod
    def backward(ctx, diff_grad, spec_grad, resi_grad):
        ...
        gb_pos_grad, gb_normal_grad, gb_kd_grad, gb_ks_grad, light_grad = _get_plugin().env_shade_bwd(
...
class OptiXContext:
    def __init__(self):
        self.cpp_wrapper = _get_plugin().OptiXStateWrapper(os.path.dirname(__file__), torch.utils.cpp_extension.CUDA_HOME)
...
def optix_build_bvh(optix_ctx, verts, tris, rebuild):
    ...
    with torch.cuda.device(verts.device):
        _get_plugin().optix_build_bvh(optix_ctx.cpp_wrapper, verts.view(-1, 3), tris.view(-1, 3), rebuild)
...
def optix_env_shade(optix_ctx, mask, ro, gb_pos, gb_normal, gb_view_pos, gb_kd, gb_ks, light, pdf, rows, cols, BSDF='pbr', n_samples_x=8, rnd_seed=None, shadow_scale=1.0):
    ...
    with torch.cuda.device(gb_pos.device):
...
def bilateral_denoiser(col, nrm, zdz, sigma):
    with torch.cuda.device(col.device):
