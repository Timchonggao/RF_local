...
def spec_gaussian_filter(res, sig):
    ...
    filter_ = torch.exp(-0.5*((sig*2*dis)**2)).unsqueeze(-1).unsqueeze(-1)
...
class DPSR(nn.Module):
    def __init__(self, res, sig, scale=True, shift=True):
        """
...
def diff_poisson_surface_recon(
    points: Float32[Tensor, "V 2"],
    normals: Float32[Tensor, "V 3"],
    *,
    smoothness: float,
    resolution: int,
) -> Tuple[Float32[Tensor, "V 3"], Int64[Tensor, "F 3"], Float32[Tensor, "V 3"]]:
    dpsr = DPSR(res=[resolution] * 3, sig=smoothness).to(points.device)
    dpsr_grid = dpsr((points[None] * 0.4 + 0.5).clamp(0, 1), normals[None])
    vertices, indices, vertex_normals = PSR2Mesh.apply(dpsr_grid)
    return vertices.view(-1, 3) * 2.5 - 1.25, indices.view(-1, 3), vertex_normals.view(-1, 3)
