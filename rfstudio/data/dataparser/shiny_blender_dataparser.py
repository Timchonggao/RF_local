...
@dataclass
class ShinyBlenderDataparser(BaseDataparser[Cameras, RGBAImages, Any]):
    ...
    def parse(
        ...
        *,
        ...
        if (path / 'train' / 'r_0_alpha.png').exists():
            images = load_masked_image_batch_lazy(
                image_filenames,
                device=device,
                scale_factor=self.scale_factor,
                masks=[path / (frame['file_path'] + "_alpha.png") for frame in meta['frames']]
            )
        else:
            images = load_masked_image_batch_lazy(
                image_filenames,
    ...
    def recognize(path: pathlib.Path) -> bool:
        paths = [
            ...
            path / 'train' / 'r_0_normal.png',
        ]
        return all([p.exists() for p in paths]) and (
            (path / 'train' / 'r_0_disp.tiff').exists() or (path / 'train' / 'r_0_alpha.png').exists()
        )
