...
@dataclass
class TensoIRDataparser(BaseDataparser[
    Cameras,
    RGBAImages,
    ...
    def parse(
        ...
        *,
        ...
        c2w = torch.stack((-c2w[:, 1, :], c2w[:, 2, :], -c2w[:, 0, :]), dim=1) # z-up to y-up
        ...
        images = load_masked_image_batch_lazy(
            [path / (frame['file_path'] + "_sunset.png") for frame in meta['frames']],
            device=device,
        ...
        albedo = load_masked_image_batch_lazy(
            [(path / frame['file_path']).parent / "albedo.png" for frame in test_meta['frames']],
            device=device,
            scale_factor=self.scale_factor,
            pbra=True,
        )
        ...
        relights = []
        envmaps = []
        for relight_name in ['bridge', 'city', 'fireplace', 'forest', 'night']:
            relights.append(load_masked_image_batch_lazy(
                [path / (frame['file_path'] + f"_{relight_name}.png") for frame in meta['frames']],
                device=device,
                scale_factor=self.scale_factor,
            ))
            envmaps.append(path.parent / f'{relight_name}.hdr')

        return cameras, images, (albedo, roughness, tuple(relights), tuple(envmaps))

    @staticmethod
    def recognize(path: pathlib.Path) -> bool:
        paths = [
            path / 'test_000' / 'rgba_sunset.png',
            path / 'train_000' / 'rgba_sunset.png',
            path / 'val_000' / 'rgba_sunset.png',
