...
from rfstudio.graphics import Cameras, DepthImages, DMTet, VectorImages
...
@dataclass
class TetWeaveTrainer(BaseTrainer):

    sdf_lr: float = 2e-3
    tet_lr: float = 3e-4
    base_decay: int = 1250
    sdf_reg_begin: float = 1.0
    sdf_reg_end: float = 0.05
    sdf_reg_decay_ratio: float = 0.25
    energy_reg: float = 0.1
    fairness_reg: float = 0.35
    ...
    num_steps_per_delaunay: int = 5
    stop_resample_at: Optional[int] = 3500
    z_up: bool = False

    def setup(self, model: TetWeave, dataset: DepthSynthesisDataset) -> ModuleOptimizers:
    ...
    def visualize(
        ...
        dmtet = DMTet(vertices=model.vertices, sdf_values=model.sdf_values, indices=model.indices)
    ...
    def before_update(self, model: TetWeave, optimizers: ModuleOptimizers, *, curr_step: int) -> None:
        model.sdf_weight = (
            self.sdf_reg_begin -
    ...
    @torch.no_grad()
    def after_update(self, model: TetWeave, optimizers: ModuleOptimizers, *, curr_step: int) -> None:
        if (
            ...
            # dump_float32_image(filename=Path('temp.png'), image=error_maps[0].cpu().clamp(0, 1).expand(-1, -1, 3))
