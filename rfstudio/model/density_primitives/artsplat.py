...
from rfstudio.graphics.math import quat2rot, rgb2sh, rot2quat, safe_normalize
...
from rfstudio.utils.tree import tree


@dataclass
class ArtSplatter(Module):
    ...
    connectivity: List[Tuple[int, int, Literal['rotating', 'sliding']]] = ...
    ...
    def __setup__(self) -> None:
        ...
        for node_from, node_to, edge_value in self.connectivity:
            self.connectivity_tree.define(node_from, node_to, edge_value)
        self.gaussians = Splats.random(
            size=self.num_random * self.connectivity_tree.num_nodes,
            sh_degree=self.sh_degree,
            random_scale=self.random_scale,
            device=self.device,
            requires_grad=True
        )
        self.part_indices = nn.Parameter(
            torch.arange(self.connectivity_tree.num_nodes).repeat(self.num_random),
            requires_grad=False,
        )
        self.joint_params = nn.Parameter(torch.tensor([0, 0, 0, 1]).float().expand(self.connectivity_tree.num_edges, 4))
    ...
    def load_state_dict(self, state_dict: Dict[str, Tensor]) -> None:
        ...
        self.joint_params = nn.Parameter(state_dict['joint_params'])
        self.part_indices = nn.Parameter(state_dict['part_indices'])
    ...
    def apply_articulation(self) -> Tuple[Float32[Tensor, "N 3"], Float32[Tensor, "N 4"]]:
        stack = [(None, self.connectivity_tree.root)]
        means = self.gaussians.means.unsqueeze(-1)
        rots = quat2rot(safe_normalize(self.gaussians.quats))
        global_transform = [(torch.eye(3).to(means), torch.zeros(3, 1).to(means))]
        while stack:
            eid, curr = stack.pop(-1)
            g_R, g_T = global_transform.pop(-1)
            if eid is not None:
                if self.connectivity_tree.edge(eid) == 'rotating':
                    R = quat2rot(safe_normalize(self.joint_params[eid, :]))
                    if self.timestamp < 0.5:
                        R = R.T
                    g_R = R @ g_R
                    g_T = R @ g_T
                elif self.connectivity_tree.edge(eid) == 'sliding':
                    T = self.joint_params[eid, :3]
                    if self.timestamp < 0.5:
                        T = -T
                    g_T = g_T + T
                else:
                    raise ValueError(self.connectivity_tree.edge(eid))
                means = torch.where(self.part_indices.view(-1, 1, 1) == curr, g_R @ means + g_T, means)
                rots = torch.where(self.part_indices.view(-1, 1, 1) == curr, g_R @ rots, rots)
            for next, eid in self.connectivity_tree.children(curr):
                stack.append((eid, next))
                global_transform.append((g_R, g_T))
        return means.squeeze(-1), rot2quat(rots)
    ...
    def render_depth(self, inputs: Cameras) -> DepthImages:
        ...
        means, quats = self.apply_articulation()
    ...
    @chains
    def as_module(self, *, field_name: Literal['joint_params']) -> Any:
