...
from rfstudio_ds.trainer import D_DiffDRTrainer, DiffDRRegularizationConfig
...
multi_view_horse_task = DS_TrainTask(
    dataset=SyntheticDynamicMultiViewCostumeDepthDataset(
    ...
    experiment=DS_Experiment(name='costume_mv_d_diffdr/horse', timestamp='test'),
    trainer=D_DiffDRTrainer(
        ...
        model_start_steps=[0,0,0,0,0], # static, poly, fourier_low, fourier_mid, fourier_high
        regularization=DiffDRRegularizationConfig(
            sdf_entropy_weight_begin = 0.2, # sdf entropy loss
            sdf_entropy_weight_end = 0.01, # todo 由于是很多个mesh 的entropy ，感觉要把这个参数调大一些
            sdf_entropy_decay_steps = 1250,
            sdf_entropy_start_step = 0,

            time_tv_weight_begin = 0.1, # curve derivative smooth loss
            time_tv_weight_end = 0.01,
            time_tv_decay_steps = 1000,
            time_tv_start_step = 0,

            sdf_eikonal_weight_begin = 0.01, # sdf eikonal loss
            sdf_eikonal_weight_end = 0.001,
            sdf_eikonal_decay_steps = 1250,
            sdf_eikonal_start_step = 0, # 开启后，大概需要占用1G显存

            curve_coeff_tv_weight_begin = 0.1, # curve coefficient tv smooth loss
            curve_coeff_tv_weight_end = 10,
            curve_coeff_tv_decay_steps = 1000,
            curve_coeff_tv_start_step = 0, # 开启后，大概需要占用3G显存
        ),
...
multi_view_toy_task = DS_TrainTask(
    dataset=SyntheticDynamicMultiViewCostumeDepthDataset(
        path=Path("data/multiview_dynamic_blender/toy/"),
        costume_padding_size=20
    ),
    model=D_DiffDR(
        geometry='DD_isocubes',
    ...
    experiment=DS_Experiment(name='costume_mv_d_diffdr/toy', timestamp='test'),
    trainer=D_DiffDRTrainer(
        num_steps=5000,
        batch_size=8,
        num_steps_per_val=50,
        ...
        regularization=DiffDRRegularizationConfig(
            ...
            time_tv_weight_begin = 0.1, # curve derivative smooth loss
            time_tv_weight_end = 0.03,
            time_tv_decay_steps = 1000,
            time_tv_start_step = 0,

            sdf_eikonal_weight_begin = 0.01, # sdf eikonal loss
            sdf_eikonal_weight_end = 1,
            sdf_eikonal_decay_steps = 1250,
            sdf_eikonal_start_step = 0, # 开启后，大概需要占用1G显存
...
class ExportModel(Task):
    ...
    def run(self) -> None:
        ...
        assert isinstance(model, D_DiffDR)
...
class EvalSceneFlow(Task):
    ...
    load: Path = Path('/data3/gaochong/project/RadianceFieldStudio/outputs/costume_mv_d_diffdr/toy/debug_flexi_v5/task.py')
    # load: Path = Path('/data3/gaochong/project/RadianceFieldStudio/outputs/costume_mv_d_diffdr/horse/debug_iso/task.py')
    step: Optional[int] = None
    ...
    def run(self) -> None:
        ...
        model.eval()
        full_batch_size = dataset.get_size(split='fix_vis') # full time size
        loader = dataset.get_orbit_vis_iter(batch_size=full_batch_size, shuffle=False, infinite=False)
        model.reg_scene_flow_smoothness_able = True
        inputs, gt_outputs, indices = next(loader)
        inputs:DS_Cameras = inputs.reset_c2w_to_ref_camera_pose(ref_camera_idx=int(len(inputs)*3/4))
        model.set_batch_gt_geometry([getattr(trainer, f'gt_geometry_fix_vis')[i] for i in indices.tolist()])
        pred_meshes, _,  = model.get_geometry(times=inputs.times)

        chamfer_list1, chamfer_list2, chamfer_list3 = [], [], []
        vis_list1, vis_list2, vis_list3 = [], [], []
        ...
        for frame in range(inputs.shape[0]):
            ...
            # --- Predicted Mesh ---
            pred_mesh = pred_meshes[frame]
            vis_list2.append(render_and_export_mesh(
        ...
        experiment.dump_images2video(subfolder='eval_scene_flow', images=vis_list2, name='pred_mesh_vis.mp4',target_mb=2,duration=5)
        experiment.dump_images2video(subfolder='eval_scene_flow', images=vis_list3,name='pred_mesh_by_sceneflow_vis.mp4',target_mb=2,duration=5)



@dataclass
class ExportInterpolateMesh(Task):
    """为训练好的 D_DiffDR 模型评估场景流，生成网格并计算场景流用在mesh上看预测效果。"""

    load: Path = Path('/data3/gaochong/project/RadianceFieldStudio/outputs/costume_mv_d_diffdr/toy/debug_flexi_v6/task.py')
    step: Optional[int] = None
    ...
    def run(self) -> None:
        ...
        time_tensor = torch.linspace(inputs.times[0].item(), inputs.times[-1].item(), steps=771).unsqueeze(1).to(inputs.times.device)
        dt = time_tensor[1] - time_tensor[0]
        pred_meshes, _,  = model.get_geometry(times=time_tensor)
        vis_list = []
        vis_list1 = []
        ...
        for frame in range(len(pred_meshes)):
            camera = inputs[0]
            
            # --- Predicted Mesh ---
            pred_mesh = pred_meshes[frame]
            vis_list.append(render_and_export_mesh(
                pred_mesh, camera, PrettyShader(True),
                experiment.dump_file_path(subfolder='exportinterpolatemesh/pred_mesh', file_name=f'{frame}.obj'),
                'exportinterpolatemesh/pred_mesh_vis', frame)
            )

            next_pred_mesh_calc = pred_mesh.get_next_frame_mesh(dt)
            vis_list1.append(render_and_export_mesh(
                next_pred_mesh_calc, camera, PrettyShader(True),
                experiment.dump_file_path(subfolder='exportinterpolatemesh/pred_mesh_by_sceneflow', file_name=f'{frame+1}.obj'),
                'exportinterpolatemesh/pred_mesh_by_sceneflow_vis', frame+1)
            )

        experiment.dump_images2video(subfolder='exportinterpolatemesh', images=vis_list, name='pred_mesh_vis.mp4',target_mb=2,duration=5)
        experiment.dump_images2video(subfolder='exportinterpolatemesh', images=vis_list1,name='pred_mesh_by_sceneflow_vis.mp4',target_mb=2,duration=5)
...
if __name__ == '__main__':
    TaskGroup(
        # train task
        mvhorse = multi_view_horse_task, # python tests_ds/models/test_d_diffdr.py mvhorse --experiment.timestamp test1
        mvtoy = multi_view_toy_task, # python tests_ds/models/test_d_diffdr.py mv_toy --experiment.timestamp test1

        # export task
        exportmodel = ExportModel(cuda=0),
        exportmesh = ExportMesh(cuda=0),
        ...
        exportinterpolatemesh = ExportInterpolateMesh(cuda=0),
