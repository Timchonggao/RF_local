...
class Extract_results(Task):
    ...
    def run(self) -> None:
        ...
        with console.status(desc='Loading Model'):
            ...
            self.dataset: RealDynamicMultiviewObjectRGBADataset = task.dataset
    ...
    def process_loader(self):
        ...
        for inputs, gt_outputs, indices in loader:
            ...
            for i in range(len(pred_rgbs)):
                ...
                self.experiment.log(f"Test view {view_num}, Frame {frame_id}: PSNR: {psnr}, SSIM: {ssim}, LPIPS: {lpips}", new_logfile='eval.txt')
                psnrs.append(psnr)
                ssims.append(ssim)
                lpipss.append(lpips)
                ...
                if view_num == 0:
                    ...
                    pred_mesh.replace_(vertices=(inv_trans @ (pred_mesh.vertices * 3).unsqueeze(-1)).squeeze(-1)) # * (3) 是把超参数scale还原回去（还原为gt的0.5范围）
                    pred_mesh.export(
        ...
        with tqdm(total=full_batch_size * 6, desc="Evaluating and Extracting results", unit="frame") as pbar:
            for inputs, gt_outputs, _ in loader:
                ...
                for i in range(len(inputs)):
                    ...
                    pred_mesh.replace_(vertices=((pred_mesh.vertices * 2/3).unsqueeze(-1)).squeeze(-1)) # * 2 是把0.5的范围映射到1
                    pred_mesh.export(
