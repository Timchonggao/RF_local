...
class NearestGridSampler(Module):
    ...
    def aggregate(
        ...
        *,
        ...
        self._grid.aggregate(positions, importances,average=average_aggregate)

    def get_density(self, positions: Float32[Tensor, "... 3"]) -> Float32[Tensor, "... 1"]:
        return self._grid.query(positions)

    def get_max_density(self, topk: int = 1) -> float:
        topk_values, topk_indices = torch.topk(self._grid.values.squeeze(1), topk)
