...
def _load_obj(
    ...
    *,
    ...
    1. add read_mtl option to control mtl read or not.
    '''
    ...
    if not mtl_override:
        for line in lines:
            if len(line) == 0:
                continue
            if line[0] == 'mtllib':
                if read_mtl is not None: # # some obj's mtl lost, if we don't want to read mtl, we ban mtl read
                    if read_mtl:
                        assert len(line) == 2
