...
from rfstudio_ds.model import D_Joint, D_Joint
...
class RegularizationConfig:
    ...
    curve_coeff_tv_start_step: Optional[int] = -1
    curve_coeff_tv_end_step: Optional[int] = -1



@dataclass
class D_JointTrainer(DS_BaseTrainer):

    geometry_lr: float = 0.003 # 用于flexicube的 deform 和 weight 参数
    geometry_lr_decay: Optional[int] = None
    ...
    def setup(
        ...
        if model.geometry == 'DD_flexicubes':
            optim_dict['geometry_deforms'] = Optimizer(
                ...
                lr=self.geometry_lr,
                lr_decay=self.learning_rate_decay_steps if self.geometry_lr_decay is None else self.geometry_lr_decay,
                max_norm=None,
                eps=self.optimizer_epsilon      
            ) 
    ...
    def _get_results(self, model: D_Joint, outputs: Tuple, gt_outputs: Union[RGBAImages, None], mode: str, visual: bool, return_pred_mesh: bool = False):
        def safe_detach(x):
        ...
        if self.mask_weight > 0:
            ...
            metrics.update({
                ...
                'real_mask_loss': safe_detach(mask_loss) / (self.mask_weight * 5.0) if model.geometry == 'DD_flexicubes' else safe_detach(mask_loss) / self.mask_weight,
            })
    ...
    def before_update(self, model: D_Joint, optimizers: DS_ModuleOptimizers, *, curr_step: int) -> None:
    ...
    def after_update(self, model: D_Joint, optimizers: DS_ModuleOptimizers, *, curr_step: int) -> None:
        self._real_learning_rates.update({
