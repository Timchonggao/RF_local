        ...
        with tqdm(total=full_batch_size * 6, desc="Evaluating and Extracting results", unit="frame") as pbar:
            for inputs, gt_outputs, _ in loader:
                ...
                for i in range(len(inputs)):
                    ...
                    self.experiment.log(f"Test view {view_num}, Frame {frame_id}: PSNR: {psnr}, SSIM: {ssim}, LPIPS: {lpips}, MAE: {mae}", new_logfile=f'{self.load.parent.parent}/baselines_eval_full/dg-mesh/eval.txt')
                    ...
                    for idx, (img, sub) in enumerate([
                        ...
                        self.experiment.dump_image(
                            f'{self.load.parent.parent}/baselines_eval_full/dg-mesh/extract/test_view{view_num}/pred/{sub}',
                            image=img, 
                            name=f'{frame_id}'
                        )
                    pbar.update(1)
        ...
        for name, imgs in video_dict.items():
            for i in range(6):
                self.experiment.dump_images2video(
                    f'{self.load.parent.parent}/baselines_eval_full/dg-mesh/extract/test_view{i}',
                    name=name, 
                    images=imgs[i*full_batch_size:(i+1)*full_batch_size], 
                    downsample=1, 
                    fps=48, 
                    duration=5
                )
        ...
        with tqdm(total=full_batch_size * 6, desc="Evaluating and Extracting results", unit="frame") as pbar:
            for inputs, gt_outputs, _ in loader:
                ...
                for i in range(len(inputs)):
                    ...
                    self.experiment.log(f"Test view {view_num}, Frame {frame_id}: PSNR: {psnr}, SSIM: {ssim}, LPIPS: {lpips}", new_logfile=f'{self.load.parent.parent}/baselines_eval_full/dg-mesh/eval.txt')
                    ...
                    if frame_id == full_batch_size - 1:
                        ...
                        for name, imgs in video_dict.items():
                            self.experiment.dump_images2video(
                                f'{self.load.parent.parent}/baselines_eval_full/dg-mesh/extract/test_view{view_num}',
                                name=name, 
                                images=imgs, 
                                downsample=1, 
                                fps=48, 
    ...
    def process_loader(self):
        ...
        with tqdm(total=full_batch_size * 6, desc="Evaluating and Extracting results", unit="frame") as pbar:
            for inputs, gt_outputs, _ in loader:
                ...
                torch.cuda.empty_cache()
        for i in range(len(chamfer_dists)):
            self.experiment.log(f"Frame {i}: Chamfer Distance: {chamfer_dists[i]}", new_logfile=f'{self.load.parent.parent}/baselines_eval_full/dg-mesh/eval.txt')
        mean_chamfer_dist = torch.stack(chamfer_dists).mean().item()
        self.experiment.log(f"Mean Chamfer Distance: {mean_chamfer_dist}")
