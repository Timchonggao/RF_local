...
for category in "${SynthoCATEGORIES[@]}"; do
    ...
    BASELINE_VISUAL="$SynthoROOT/0_baselines/sc-gs/$category/train/ours_80000/renders"
    BASELINE_MESH="$SynthoROOT/0_baselines/sc-gs/$category/train/ours_80000/mesh"

    CUDA_VISIBLE_DEVICES=1 python "$SCRIPT_PATH" evalcmu \
        --load "$LOAD_PATH" \
