...
for category in "${SynthoCATEGORIES[@]}"; do
    ...
    BASELINE_VISUAL="$SynthoROOT/baselines/deformable-3dgs/$category/test/ours_40000/renders"
    BASELINE_MESH="$SynthoROOT/baselines/deformable-3dgs/$category/train/ours_40000/mesh"

    CUDA_VISIBLE_DEVICES=7 python "$SCRIPT_PATH" evaldiva \
        --load "$LOAD_PATH" \
